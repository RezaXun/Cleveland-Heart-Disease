...
with tab1 :
  ...
  slope_sb = st.sidebar.selectbox(label=":violet[slope of the peak exercise ST segment]",
  ...
  ca = st.sidebar.number_input(label=':violet[Number of major vessels]',
                                        min_value=df['slope'].min(),
                                        max_value=df['slope'].max())
  st.sidebar.write(f":orange[Min]: {df['slope'].min()} :red[Max]: {df['slope'].max()}")
  st.sidebar.write("")

  thal_sb = st.sidebar.selectbox(label=":violet[Thalassemia]",
